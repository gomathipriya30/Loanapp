...
function AddLoan() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/admin/add-loan', formData);
      setMessage(response.data.message); setMessageType('success');
      setTimeout(() => navigate('/admin/dashboard/loans'), 1500); // Go back to list
    } catch (err) {
      setMessage(err.response?.data?.message || 'Failed to add loan.');
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-dark-slate mb-6 pb-2 border-b-2 border-gray-200">Add New Loan Type</h2>
      <div className="bg-white p-6 md:p-8 rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" className="w-full md:w-auto px-6 py-2 bg-accent-orange text-white font-bold rounded-lg hover:bg-accent-orange-dark">Add Loan</button>
        </form>
      </div>
    </div>
  );
