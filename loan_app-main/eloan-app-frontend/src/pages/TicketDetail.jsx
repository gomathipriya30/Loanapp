...
function TicketDetail() {
  ...
  return (
    <div>
      <Link to="/dashboard/support" className="text-accent-orange hover:underline mb-4 block">&larr; Back to all tickets</Link>
      ...
      <div className="space-y-4 mt-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border">
          <p className="text-sm text-gray-500">You said on {new Date(ticket.created_at).toLocaleString()}</p>
        ...
        {replies.map((reply) => (
          <div key={reply.id} className={`p-4 rounded-lg shadow-sm ${reply.replier_role === 'admin' ? 'bg-green-50 border border-green-200' : 'bg-white border'}`}>
            <p className="text-sm font-semibold text-gray-700">
              {reply.replier_role === 'admin' ? `${reply.replier_name} (Support)` : 'You'}
      ...
      {ticket.status === 'open' && (
        <form onSubmit={handleReply} className="mt-6">
          ...
          <textarea
            ...
            className="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-green"
            placeholder="Type your reply..."
          ></textarea>
          <button type="submit" className="mt-2 px-6 py-2 bg-primary-green text-white font-bold rounded-lg hover:bg-primary-green-dark">
            Send Reply
          </button>
        </form>
      )}
